...
ggplot(data)+
  ...
  labs(x="log(Nombre de taxon)",y="log(Nombre d'associations)",
       subtitle="Med")
...
# Fonction pour générer une matrice nulle
generate_null_matrix <- function(mat) {
  n <- nrow(mat)  # Taille de la matrice
  # Extraire les indices de la moitié triangulaire supérieure (sans diagonale)
  upper_indices <- which(upper.tri(mat))
  
  # Extraire les valeurs de la moitié triangulaire supérieure
  upper_values <- mat[upper_indices]
  
  # Permuter ces valeurs aléatoirement
  permuted_values <- sample(upper_values)
  
  # Créer une nouvelle matrice nulle
  null_mat <- matrix(0, n, n)
  
  # Placer les valeurs permutées dans la moitié triangulaire supérieure
  null_mat[upper_indices] <- permuted_values
  
  # Compléter la matrice pour qu'elle soit symétrique
  null_mat <- null_mat + t(null_mat)
  
  # Retourner la matrice nulle
  return(null_mat)
}
...
edge_list_null <- as_data_frame(null_net, what = "edges")
...
data_null_reseaux[i,1] <- nrow(filter(edgelist_genus_null, link_genus == "Bacillariophyceae-Bacillariophyceae")) # Nombre de relation Bac-Bac
data_null_reseaux[i,2] <- nrow(filter(edgelist_genus_null, link_genus == "Bacillariophyceae-Bacillariophyceae"))/nrow(edgelist_genus_null) # Pourcentage de relation Bac-Bac
data_null_reseaux[i,3] <- nrow(filter(edgelist_genus_null, link_genus == "Bacillariophyceae-Dinophyceae")) # Nombre de relation Bac-Dino
data_null_reseaux[i,4] <- nrow(filter(edgelist_genus_null, link_genus == "Bacillariophyceae-Dinophyceae"))/nrow(edgelist_genus_null) # Pourcentage de relation Bac-Dino
data_null_reseaux[i,5] <- nrow(filter(edgelist_genus_null, link_genus == "Dinophyceae-Dinophyceae")) # Nombre de relation Dino-Dino
data_null_reseaux[i,6] <- nrow(filter(edgelist_genus_null, link_genus == "Dinophyceae-Dinophyceae"))/nrow(edgelist_genus_null) # Pourcentage de relation Dino-Dino
data_null_reseaux[i,7] <- nrow(filter(edgelist_genus_null, link_genus != "Dinophyceae-Dinophyceae" & link_genus != "Bacillariophyceae-Dinophyceae" & link_genus != "Bacillariophyceae-Bacillariophyceae" )) # Nombre d'autres associations
data_null_reseaux[i,8] <- nrow(filter(edgelist_genus_null,link_genus != "Dinophyceae-Dinophyceae" & link_genus != "Bacillariophyceae-Dinophyceae" & link_genus != "Bacillariophyceae-Bacillariophyceae" ))/nrow(edgelist_genus_null) # Pourcentage d'autres relations
data_null_reseaux[i,9] <- nrow(edgelist_genus_null)

data_null_reseaux[i,10] <- nrow(filter(taxons,Classe == "Bacillariophyceae")) # Nombre de Bacillariophyceae
data_null_reseaux[i,11] <- nrow(filter(taxons,Classe == "Bacillariophyceae"))/nrow(taxons) # Pourcentage de Bacillariophyceae
data_null_reseaux[i,12] <- nrow(filter(taxons,Classe == "Dinophyceae")) # Nombre de Dinophyceae
data_null_reseaux[i,13] <- nrow(filter(taxons,Classe == "Dinophyceae"))/nrow(taxons) # Pourcentage de Dinophyceae
data_null_reseaux[i,14] <- nrow(filter(taxons,Classe != "Bacillariophyceae" & Classe != "Dinophyceae")) # Nombre d'autres taxons
data_null_reseaux[i,15] <- nrow(filter(taxons,Classe != "Bacillariophyceae"& Classe != "Dinophyceae"))/nrow(taxons) # Pourcentage d'autres taxons
data_null_reseaux[i,16] <- nrow(taxons)
