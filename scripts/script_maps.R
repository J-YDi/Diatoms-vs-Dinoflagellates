...
ggplot() + geom_polygon(data = Worldmap, aes(x = long, y = lat, group = group), fill = "gray", color = 'gray10', size = .25)+
  ...
  theme_gdocs()+
  geom_point(data = data, aes(x = lon, y = lat,colour=region), size =8)+
  ...
  geom_text(aes(x = 5, y = 42, label = "Mediterranean sea"), stat = 'unique', size = 4,color = "lightblue4",fontface="italic",angle=8) +
