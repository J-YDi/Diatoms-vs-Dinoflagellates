# Script Suite Stage JY Dias # 13/01/2024
...
# Pourcentage of association types 
...
# Ponderate pourcentage of association types 
...
# Create a circular plot to the the differences in composition and association before, during and after a bloom
...
# Create all the df needed for the different visualizations
# Associations 
...
# Compute the cumulative percentages (top of each rectangle)
Associations_med_after$ymax <- cumsum(Associations_med_after$value)
# Compute the bottom of each rectangle
...
# Compute the cumulative percentages (top of each rectangle)
Associations_manche_after$ymax <- cumsum(Associations_manche_after$value)
# Compute the bottom of each rectangle
...
# Compute the cumulative percentages (top of each rectangle)
Associations_atlantic_after$ymax <- cumsum(Associations_atlantic_after$value)
# Compute the bottom of each rectangle
...
# Compute the cumulative percentages (top of each rectangle)
Compositions_med_after$ymax <- cumsum(Compositions_med_after$value)
# Compute the bottom of each rectangle
...
# Compute the cumulative percentages (top of each rectangle)
Compositions_manche_after$ymax <- cumsum(Compositions_manche_after$value)
# Compute the bottom of each rectangle
...
# Compute the cumulative percentages (top of each rectangle)
Compositions_atlantic_after$ymax <- cumsum(Compositions_atlantic_after$value)
# Compute the bottom of each rectangle
...
med_before <- ggplot() +
  geom_rect(data = Compositions_med_before, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=name)) +
  geom_rect(data = Associations_med_before, aes(ymax=ymax+0.01, ymin=ymin, xmax=2.9, xmin=2, fill=name)) +
  geom_text( data = Compositions_med_before,x=4.7, aes(y=labelPosition, label=label, color=name), size=3) + # x here controls label position (inner / outer)
  geom_text( data = Associations_med_before,x=1.2, aes(y=labelPosition, label=label, color=name), size=3) + # x here controls label position (inner / outer)
  scale_fill_manual(values = c(
    "P_bac" = "#1f77b4", "P_dino" = "green",
...
# Same but with ponderate associations for diatoms
blooms_summary <- summarise(group_by(blooms_longer,region,Moment,name,Bloom_Phylum),value=mean(value,na.rm=T))
...
# Same but with ponderate associations for dinoflagellates
blooms_summary <- summarise(group_by(blooms_longer,region,Moment,name,Bloom_Phylum),value=mean(value,na.rm=T))
